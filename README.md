...
泰坦尼克生存预测是Kaggle专门设置的一个竞赛**入门练习**，它以竞赛形式体现，但不计成绩，竞赛开始至今已经两年，目前有9531支团队参赛，我两天前提交的成绩还在6%的位置，两天后就后退到8%，可见目前该竞赛还很活跃。竞赛目标是根据871条训练数据生成模型，预测400多条测试集数据的生存情况。

Kernels是大家交流发现的地方，基于此竞赛的学习特性，本Kernels中有超过40篇铜牌及以上教学性质的notebook，可谓争奇斗艳，本人Kaggle/ML新手，故趁着假期在kernels中选取5篇有各自特点的notebook进行学习并将其特点记录下来以备之后查阅。
...
* [Titanic 0.82 - 0.83](https://www.kaggle.com/konstantinmasich/titanic-0-82-0-83) 提交的分0.83
...
Kaggle的notebook本来可以fork后直接云端运行，但是个人觉得没有本机的jupyter notebook方便，其中前三篇都是在本机的jupyter notebook中进行试验。[github地址](https://github.com/dashjim/kaggle-titanic)
...
* 集成学习-ensemble，之前大致了解原理，但从没真正搞明白，现在才明白这是个大的概念，有些是独立的算法，如随机森林是集成了多个决策树。有些是将不同类的算法预测结果再次进行提升，如voting，stacking。
* 有文章指出本文使用的stacking可能是所有ensemble方法中效果最好的。
* stacking具体过程为先用不同算法基于Kfold进行训练和CV。将CV时的预测值保存下来，每个算法的预测结果为一列（一个feature），多个算法的多组预测值（基于训练集）合在一起即是tier1的输出，用作tier2算法的输入，来训练一个新的模型（m2）。tier1还要预测测试集，所产生的多组预测结果通过`mean()`来缩小为一组结果(test2)。新的模型（m2）对test2进行预测的结果即为最终提交结果。
...
* 介绍Kaggle的public score和private score，给出有些情况下会出现过拟合public score的情况。
...
* 和前面notebook不同做分段处理时Age，Fare都是使用基于频率的`qcut()`。
* 唯一指出特征缩放的重要性，使用特这缩放`Standard Scaler`
...
* 好模型不如好数据。留言中有关于为啥测试集public score这么低的讨论，似乎指向过度训练。
...
* 尽管绝大多数参赛者使用了交叉验证（Cross Validation）来防止过拟合，但所有notebook都存在训练集得分大幅超越提交实际得分的情况。说明测试集的分布与训练集存在较大的不一致。第五个notebook最后的讨论中有提及，但还没仔细看。
* 是否可以使用深度神经网络自行生成大量组合特征（数百量级）并提取有效特征？
* 高分只是竞赛的结果，对高分的追求过程中的学习，思考，假设，试验才是精华。
